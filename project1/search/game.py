...
class Agent: #抽象类
    ...
    def getAction(self, state):
        ...
        """
        raiseNotDefined() #子类必须实现这个方法，否则会报错
...
leftDirection=Directions.LEFT[currentDirection]
rightDirection=Directions.RIGHT(currentDirection)
...
class AgentState: #用于跟踪游戏中每个agent的动态信息
    """
...
class Grid:
    ...
    def deepCopy(self):  #深度拷贝就是创建一个新的对象，与原来的独立
        return self.copy()

    def shallowCopy(self): #浅度拷贝就是类似于引用
        g = Grid(self.width, self.height)
        g.data = self.data
        return g

    def count(self, item =True ): #统计True的数量
        return sum([x.count(item) for x in self.data])

    def asList(self, key = True):  #返回网格中所有等于指定值（默认为 True）的位置列表
        list = []
    ...
    def packBits(self): #将网格数据压缩为位表示，返回一个包含宽度、高度和压缩数据的元组
        """
    ...
    def _unpackBits(self, bits): #解压缩，恢复网格数据
        """
