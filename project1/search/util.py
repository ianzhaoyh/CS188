...
class PriorityQueue:
    ...
    def push(self, item, priority): #insert an item into the queue with a priority
        entry = (priority, self.count, item)
        heapq.heappush(self.heap, entry)
        self.count += 1

    def pop(self):
        (_, _, item) = heapq.heappop(self.heap) #pop the smallest item from the queue, return 
        return item

    def isEmpty(self):
        return len(self.heap) == 0

    def update(self, item, priority): #update the priority of an item
